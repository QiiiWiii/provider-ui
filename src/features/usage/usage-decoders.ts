...
export function decodeUsageRequestDetail(value: unknown): UsageRequestDetail {
  ...
  const label = 'usage request attempt'
  const attempt = requireRecord(record.attempt, label)
  const cost = requireRecord(attempt.cost, `${label} cost`)
  const components = requireRecord(cost.components, `${label} cost components`)
  ...
  return {
    ...
    attempt: {
      cost: {
        ...
        inputUsd: nullableDecimalAmount(components.input_usd, `${label} input cost`),
        outputUsd: nullableDecimalAmount(components.output_usd, `${label} output cost`),
...
function decodeCostTotals(value: unknown): UsageCostTotals {
  ...
  return {
    usd: nullableDecimalAmount(record.usd, 'usage cost'),
  }
}
